<template>
  <view class="flex flex-col h-screen overflow-hidden">
    ...
    <view class="flex-1 min-h-0 flex">
      <view class="w-20 bg-white h-full">
        <view class="text-sm text-center text-gray-500">
          <view class="py-3" :class="[currentIndex === 0 ? 'bg-gray-50' : '']">推荐</view>
          <view class="py-3" v-for="i in 10" :key="i" :class="[currentIndex === parseInt(i+1) ? 'bg-gray-50' : '']">建筑类</view>
        </view>
      </view>
      <view class="flex-1 min-w-0 py-1">
        <scroll-view :scroll-y="true" style="height: calc(100vh - 3rem)" @scroll="handleScroll">
          <view class="px-3 pb-64">
            <view class="mb-4 category-item">
              <view class="text-sm text-gray-900 py-2">推荐</view>
              <view class="grid grid-cols-2 gap-3">
                <view class="bg-white px-1 py-3 text-center rounded-md shadow-sm">
                  ...
                  <view class="py-1 text-xs text-gray-900">中级会计师</view>
                </view>
                <view class="bg-white px-1 py-4 text-center rounded-md shadow-sm">
                  <view class="py-1">
                    <view class="inline-block w-5 h-5 border-2 border-solid border-gray-900"></view>
                  </view>
                  <view class="py-1 text-xs text-gray-900">二级建造师</view>
                </view>
              </view>
            </view>
            <view class="mb-4 category-item" v-for="i in 10" :key="i">
              <view class="text-sm text-gray-900 py-2">建筑类</view>
              <view class="grid grid-cols-2 gap-3">
                <view class="bg-white px-1 py-3 text-center rounded-md shadow-sm">
                  <view class="py-1">
                    <view class="inline-block w-5 h-5 border-2 border-solid border-gray-900"></view>
